...
/*
	This method initializes all elements of the matrix to 0.
*/
void
...
/*
	This method displays a matrix on the console.
*/
void
...
split(int o_size, int size, int O[][o_size], int N[][size], int si, int sj) {
	int i1 =0, i2 = si, j1 = 0, j2 = sj;
		...
		j2 = sj;
		for(j1 = 0; j1 < size; j1++) {
			//printf("\nj2 : %d, i2 : %d, O[i2][j2] : %d, i2 : %d, j1 : %d",j2, i2, O[i2][j2], i1, j1);
			N[i1][j1] = O[i2][j2];
			j2++;
		}
		i2++;
...
/*
	...
	If results are identical then method returns true, otherwise it returns false.
*/
int
...
int main(){
	...
	if(((n == 1) || (n & (n - 1)))){
		printf("Please enter a valid number !! (n = 2^k)");
		printf("\n");
		return 1;
	}
